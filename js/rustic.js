document.addEventListener("DOMContentLoaded", function () {
  ...
  homepages = document
    .querySelectorAll(".homepage")
    .forEach(function (homepage) {
...
function get_gallery() {
  ...
  random_likes = document
    .querySelectorAll(".like-count")
    .forEach(function (random_like) {
...
async function get_dynamo_menu() {
  ...
  if (menu == null) {
    ...
    let data = await fetch(
      "https://lp30qsvq56.execute-api.ap-southeast-2.amazonaws.com/prod",
      requestOptions
    )
  ...
  for (var i = 0; i < menu.length; i++) {
    ...
    if (menu[i].category != current_category) {
      ...
      if (menu[i].type == "FOOD") {
        ...
        all_menu_tab[tab_item_count].setAttribute(
          "class",
          `each-menu-tab middle ${menu[i].category}`
        );
        all_menu_tab[tab_item_count].setAttribute(
          "onclick",
          `get_menu_section('${menu[i].category}')`
        );

        if (menu[i].category === "ALL-DAY-BREAKFAST") {
          all_menu_tab[tab_item_count].innerHTML =
            '<div class="middle">breakfast</div>';
        } else {
          all_menu_tab[tab_item_count].innerHTML = `<div class="middle">${menu[
            i
          ].category
            .replace(/-/g, " ")
            .toLowerCase()}</div>`;
        ...
        all_drink_tab[tab_item_count_drink].setAttribute(
          "class",
          `each-menu-tab-drink middle ${menu[i].category}`
        );
        all_drink_tab[tab_item_count_drink].setAttribute(
          "onclick",
          `get_menu_drink('${menu[i].category}')`
        );
        all_drink_tab[
          tab_item_count_drink
        ].innerHTML = `<div class="middle">${menu[i].category
          .replace(/-/g, " ")
          .toLowerCase()}</div>`;
        tab_item_count_drink++;
      }
      ...
      if (i == 0) {
        ...
        if (menu[i].type == "FOOD") {
          ...
          all_menu_section[section_item_count].setAttribute(
            "class",
          ...
          all_drink_section[section_item_count_drink] =
            document.createElement("div");
          all_drink_section[section_item_count_drink].setAttribute(
            "class",
      ...
      // if reach new category midway through menu
        ...
        if (current_type == "FOOD") {
          all_menu_section[section_item_count].innerHTML =
            each_section_holder.innerHTML;
          each_section_holder.innerHTML = "";
          section_item_count++;
        } else {
          all_drink_section[section_item_count_drink].innerHTML =
            each_section_holder.innerHTML;
          each_section_holder.innerHTML = "";
...
function get_menu_section(section) {
  ...
  for (var i = 0; i < menu_section.length; i++) {
    ...
    if (
      menu_section[i].style.display == "flex" &&
      menu_section[i].getAttribute("id") !== section
    ) {
      ...
      current_section.addEventListener("animationend", () => {
        ...
        document.querySelector(
          `.${current_section.getAttribute("id")}`
...
function get_menu_drink(section) {
  ...
  for (var i = 0; i < drink_section.length; i++) {
    ...
    if (
      drink_section[i].style.display == "flex" &&
      drink_section[i].getAttribute("id") != section
    ) {
...
async function get_s3_pictures() {
  ...
  if (gallery == null) {
    ...
    // make API call with parameters and use promises to get response
    let data = await fetch(
      "https://hau5jajds9.execute-api.ap-southeast-2.amazonaws.com/prod",
      requestOptions
    )
      .then((response) => response.text())
      .then((result) => {
...
function update_like(like_icon) {
  ...
  let like_count = document.getElementById(
    `${image_id.replace("_icon", "_count")}`
  );
...
function send_message() {
  ...
  fetch(
    "https://49jor2d89l.execute-api.ap-southeast-2.amazonaws.com/prod",
    requestOptions
  )
    .then((response) => response.text())
    .then((result) => {
